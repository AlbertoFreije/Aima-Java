...
public class GeneticAlgorithm<A> {
	...
	//CAMBIO esto hay que cambiar algo arriba que no veia EJERCICIO 1
	public double averageFitness(Collection<Individual<A>> population, FitnessFunction<A> fitnessFn) {
		...
		for(Individual<A> individual : population) {
			accValue += fitnessFn.apply(individual);
		}
		return accValue / population.size();
	}
	...
	protected List<Individual<A>> nextGeneration(List<Individual<A>> population, FitnessFunction<A> fitnessFn, Individual<A> bestBefore) {
		...
		for (int i = 0; i < population.size(); i++) {
			...
			// child <- REPRODUCE(x, y)
			Individual<A> child;
			if (random.nextDouble() <=  crossoverProbability) { //ejercicio 5
		...
		//elitise
		newPopulation.add(bestBefore); //ejercicio 6
	...
	public Individual<A> mutate3(Individual<A> cromosome) {
